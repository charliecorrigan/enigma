...
class TestEnigma < Minitest::Test
  ...
  def test_decrypt_method_return_value_characteristics_with_one_argument
    skip
    enigma = Enigma.new
    assert_equal String, enigma.decrypt("z9fzjp").class
    assert_equal 6, enigma.decrypt("z9fzjp").length
  end

  def test_if_decrypt_method_returns_encrypted_text
    skip
    enigma = Enigma.new
    result = enigma.decrypt("z9fzjp", "12345", 260317)
